...
import { useWorkout, weeklyMinutes, type WorkoutRecord } from '../context/WorkoutContext';
...
export default function WorkoutTimeDetailScreen({ onBack }: Props) {
  const insets = useSafeAreaInsets();
  const { history } = useWorkout();

  const minThisWeek = weeklyMinutes(history);
  const allTimeMinutes = Math.round(
    history.reduce((sum, r) => sum + r.durationSeconds, 0) / 60,
  );
  const thisWeekWorkouts = useMemo(() => getThisWeekWorkouts(history), [history]);
  const avgPerWorkout =
    thisWeekWorkouts.length > 0
      ? Math.round(minThisWeek / thisWeekWorkouts.length)
      : 0;
      ...
      >
        <View style={styles.heroCard}>
          ...
          <Text style={styles.heroValue}>
            {minThisWeek > 0 ? `${minThisWeek}m` : '0m'}
          </Text>
          <Text style={styles.heroHint}>
            {thisWeekWorkouts.length > 0
              ? `${thisWeekWorkouts.length} session${thisWeekWorkouts.length !== 1 ? 's' : ''} logged`
              : 'Start a workout to track your time'}
          </Text>
        </View>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statCard}>
            ...
            <Text style={styles.statValue}>
              {avgPerWorkout > 0 ? `${avgPerWorkout}m` : '—'}
            </Text>
          </View>
          <View style={styles.statCard}>
            <Text style={styles.statLabel}>All Time</Text>
            <Text style={styles.statValue}>
              {allTimeMinutes > 0 ? formatDuration(allTimeMinutes * 60) : '0m'}
            </Text>
          </View>
        </View>
        ...
        {thisWeekWorkouts.length > 0 ? (
            ...
            {thisWeekWorkouts.map((record) => {
              ...
              return (
                ...
                >
                  ...
                  <Text style={styles.workoutTime}>{mins > 0 ? `${mins}m` : '—'}</Text>
                </View>
              );
            })}
          </>
          ...
          <View style={styles.emptyCard}>
            ...
            <Text style={styles.emptyBody}>
              Your weekly workout time will show up here after your first session.
            </Text>
          </View>
        )}
